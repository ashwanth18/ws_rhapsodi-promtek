...
int main(int argc, char** argv) {
    ...
    assert(t.getRobotModel()->getName() == "niryo_ned3pro");
    ...
    // Strategy 1: Minimize path length
    {
      auto connect{ std::make_unique<stages::Connect>(
           "path length", stages::Connect::GroupPlannerVector{ { "arm", pipeline } }) };
    ...
    {
      auto connect{ std::make_unique<stages::Connect>(
           "trajectory duration", stages::Connect::GroupPlannerVector{ { "arm", pipeline } }) };
    ...
    {
      auto connect{ std::make_unique<stages::Connect>(
           "eef motion", stages::Connect::GroupPlannerVector{ { "arm", pipeline } }) };
      connect->setCostTerm(std::make_unique<cost::LinkMotion>("hand_link")); // Distance traveled by the link in meters
      alternatives->add(std::move(connect));
    ...
    {
      auto connect{ std::make_unique<stages::Connect>(
           "elbow motion", stages::Connect::GroupPlannerVector{ { "arm", pipeline } }) };
      connect->setCostTerm(std::make_unique<cost::LinkMotion>("elbow_link"));
      alternatives->add(std::move(connect));
    ...
    // In our goal setup, we want the robot's arm to be in the 'goal' position.
    auto goal_scene{ scene->diff() };
    goal_scene->getCurrentStateNonConst().setToDefaultValues(robot_state.getJointModelGroup("arm"), "goal");
    std::cout << "Created goal scene with 'goal' position" << std::endl;
